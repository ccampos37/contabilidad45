...
Friend Class frmMntEntidad
    ...
    Sub ConfiguraForm()
        ...
        'Me.Width =  TwipsToPixelsX(650)
        'Me.Height =  TwipsToPixelsY(5925)
    End Sub
    ...
    Sub CargaLista()
        ...
        ListView1.Columns.Add("", "Codigo Tipo Analítico", CInt(((ListView1.Width) / 3)))
        ListView1.Columns.Add("Descripción Tipo Analítico", CInt(((ListView1.Width) * 2 / 3)), System.Windows.Forms.HorizontalAlignment.Center)
    ...
    Sub EditarValores()
        Dim i, n As Short
        Dim txt As TextBox() = New TextBox() {txt0, txt1, txt2, txt3, txt4}
    ...
    Function ValidaData() As Boolean
        ...
        '    If Len(Trim$(txt0.Text)) <> 11 And Trim$(txt0.Text) <> "00" Then
        ...
        '  If txt4.Text <> Empty And Len(Trim$(txt4.Text)) <> 11 Then
        ...
        If Not IsNothing(txt4.Text) Then
            SQL = "SELECT count(entidadruc) FROM ct_entidad WHERE entidadruc='" & txt4.Text & "'"
            rsX = New ADODB.Recordset
            rsX = VGCNx.Execute(SQL)
            If modoedit = True And ESNULO(rsX.Fields(0), 0) = 1 Then
                ValidaData = True
                SQL = "SELECT count(entidadruc) FROM ct_entidad WHERE entidadruc='" & txt4.Text & "'"
                SQL = SQL & " AND entidadcodigo<>'" & txt0.Text & "'"
                rsX = VGCNx.Execute(SQL)
    ...
    Sub GrabaData()
        ...
        If modoinsert = True Then
            ...
            SQL = SQL & "VALUES ('" & UCase(txt0.Text) & "','" & UCase(txt1.Text) & "','" & UCase(txt2.Text) & "','" & txt3.Text & "','" & txt4.Text & "','" & xVarCbo & "','" & VGUsuario & "','" & Today & "')"
            VGCNx.BeginTrans()
            VGCNx.Execute(SQL)
        ...
        ElseIf modoedit = True Then
            SQL = "UPDATE CT_ENTIDAD SET entidadrazonsocial='" & Trim(UCase(txt1.Text)) & "',"
            SQL = SQL & "entidaddireccion='" & Trim(UCase(txt2.Text)) & "',"
            SQL = SQL & "entidadtelefono='" & txt3.Text & "',"
            SQL = SQL & "entidadruc='" & txt4.Text & "',"
            SQL = SQL & "entidadtipocontri='" & xVarCbo & "',"
            SQL = SQL & "usuariocodigo='" & VGUsuario & "',fechaact=getdate() "
            SQL = SQL & "WHERE entidadcodigo='" & txt0.Text & "'"
            VGCNx.BeginTrans()
            VGCNx.Execute(SQL)
...
X:
    ...
    Sub MuestraCheckTipoAnalitico()
        ...
        Dim i As Integer
        Dim SQL As String
        SQL = "select tipoanaliticocodigo,analiticocodigo FROM ct_analitico WHERE entidadcodigo='" & txt4.Text & "'"
        rsX = VGCNx.Execute(SQL)
    ...
    Sub DeleteCheckTipoAnalitico()
        Dim SQL As String
        SQL = "DELETE FROM ct_analitico WHERE entidadcodigo='" & txt0.Text & "'"
        VGCNx.Execute(SQL)
    End Sub
    ...
    Sub GrabaCheckTipoAnalitico()
        ...
        For i = 0 To ListView1.Items.Count - 1
            If ListView1.Items.Item(i).Checked = True Then
                xCodAnalitico = Trim(txt0.Text) & Trim(ListView1.Items.Item(i).Text)
                ...
                If rsX.Fields(0).Value = 0 Then
                    ...
                    SQL = SQL & "VALUES ('" & xCodAnalitico & "','" & Trim(txt0.Text) & "','" & Trim(ListView1.Items.Item(i).Text) & "','" & VGUsuario & "','" & Today & "')"
                    VGCNx.Execute(SQL)
                End If
            End If
        Next
    ...
    Private Sub txt_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles txt.Leave
        ...
        If Index = 0 And modoedit = False Then txt4.Text = txt0.Text
    End Sub
...
X:
    ...
    Private Sub txt0_TextChanged(sender As Object, e As EventArgs) Handles txt0.TextChanged
