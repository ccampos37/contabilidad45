		...
		Do While Not rs.EOF
			...
			If rs.Fields("detcomprobdebe").Value + rs.Fields("detcomprobhaber").Value > 0 Then
				With rs
					...
					'Campo 7
                    reg = reg & Format(rs.Fields("detcomprobdebe").Value, "00000000.00") & "|"
                    reg = reg & Format(rs.Fields("detcomprobhaber").Value, "00000000.00") & "|1|"
                End With
    ...
    Public Sub GeneraArchivoCompras(ByRef rs As ADODB.Recordset, ByRef Archivo As String, ByRef li_arc1 As Short)
        ...
        Do While Not rs.EOF
            ...
            With rs
                ...
                dato2 = Format(.Fields("detcomprobtipocambio").Value, "0.000")
                ...
                dato1 = "0"
                dato2 = Format(Year(rs.Fields("detcomprobfechaemision").Value), "0000") & Format(Month(rs.Fields("detcomprobfechaemision").Value), "00")
    ...
    Public Sub GeneraArchivoVentas(ByRef rs As ADODB.Recordset, ByRef Archivo As String, ByRef li_arc1 As Short)
        ...
        Do While Not rs.EOF
            ...
            With rs
                ...
                dato1 = Format(.Fields("detcomprobtipocambio").Value, "0.000")
...
err1:
    ...
    Public Sub GeneraMayor(ByRef dato As String)
        ...
        If Mid(Archivo, 30, 1) = "2" Then
            ...
            Exit Sub
        End If
        MesAnt = Format(Val(VGParamSistem.Mesproceso) - 1, "00")
        VGCommandoSP = New ADODB.Command
        VGCommandoSP.let_ActiveConnection(VGGeneral)
        VGCommandoSP.CommandType = ADODB.CommandTypeEnum.adCmdStoredProc
            ...
            If RTrim(rs.Fields("cabcomprobnumero").Value) <> "" And (rs.Fields("detcomprobdebe").Value + rs.Fields("detcomprobhaber").Value) > 0 Then
                With rs
                    If cuenta <> rs.Fields("cuentacodigo").Value Then
                        ...
                        reg = CStr(CDbl(reg) + IIf(rs.Fields("saldoini").Value > 0, Format(rs.Fields("saldoini").Value, "00000000.00"), "0.00") + CDbl("|"))
                        'Campo 7
                        reg = CStr(CDbl(reg) + IIf(rs.Fields("saldoini").Value < 0, Format(rs.Fields("saldoini").Value * -1, "00000000.00"), "0.00") + CDbl("|") + CDbl("1") + CDbl("|"))
                    ...
                    '   campo 4
                    reg = reg & fecha & "|" + rs.Fields("detcomprobglosa").Value + "|" + Format(rs.Fields("detcomprobdebe").Value, "00000000.00") + "|"
                    'Campo 7
                    reg = reg & Format(rs.Fields("detcomprobhaber").Value, "00000000.00") & "|" & "1" & "|"
