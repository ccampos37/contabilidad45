...
X: 
	...
	Function ValidaData() As Boolean
		...
		If modoinsert = True And dllgen.VerificaDatoExistente(VGCNx, SQL) > 0 Then
            MsgBox("El Correlativo Nº " & txt(0).CtlText & " existe en la Plantilla actual", MsgBoxStyle.Information, Text)
			txt(0).Focus()
			ValidaData = False
			Exit Function
		End If
		...
		VGvardllgen = New dllgeneral.dll_general
        If Not IsNothing(Ctr_Ayuda5.xclave) And (CInt(VGvardllgen.ESNULO(txt(2).CtlText, 0)) <= 0) = True Then
	...
	Sub GrabarData()
		...
		If modoinsert = True Then
            ...
            SQL = SQL & VGvardllgen.ESNULO(txt(2).CtlText, 0) & ",'" & txt(3).CtlText & "'," & ChkAjuste.CheckState & ",'" & VGUsuario & "','" & VB6.Format(Today, "dd/mm/yyyy") & "')"
		...
		ElseIf modoedit = True Then 
            ...
            SQL = SQL & "plantillaasientovalorigv=" & VGvardllgen.ESNULO(txt(2).CtlText, 0) & ",plantillaasientocomodin='" & txt(3).CtlText & "',plantillaasientoctaajuste=" & ChkAjuste.CheckState & ",usuariocodigo='" & VGUsuario & "',fechaact='" & VB6.Format(Today, "dd/mm/yyyy") & "'"
